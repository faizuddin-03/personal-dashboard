...
- Location: tests\service-hub\specs\bo-calendar-limits.spec.ts:195:7
  ...
  - table [ref=e254]:
    - rowgroup [ref=e255]:
      - row "Best Compatible With Mozilla Firefox V36.0.4 Copyright © 2026 eAuto Sdn Bhd (676967-T) All Rights Reserved. Contact Us | Terms & Conditions | Privacy" [ref=e256]:
        ...
        - cell [ref=e262]:
          - img [ref=e263]
  - dialog "Reschedule Appointment" [active] [ref=e265]:
    - generic [ref=e267]: Reschedule Appointment
    - generic [ref=e268]:
      - generic [ref=e269]:
        - generic [ref=e270]: "Company Name:"
        - text: 193 AUTO TRADING
      - generic [ref=e271]:
        - generic [ref=e272]: "Installation Status:"
        - text: Pending
      - generic [ref=e273]:
        - generic [ref=e274]: "Installation Service Reference No.:"
        - text: SR67000001
      - generic [ref=e275]:
        - generic [ref=e276]: "Current Appointment Date:"
        - text: 20-07-2026 10:00am - 12:00pm
      - generic [ref=e277]:
        - generic [ref=e278]: "New Appointment Date*:"
        - textbox [ref=e279]
      - generic [ref=e280]:
        - generic [ref=e281]: "Time Slot*:"
        - generic [ref=e282]:
          - generic [ref=e283]:
            - radio "10:00am - 12:00pm" [ref=e284]
            - text: 10:00am - 12:00pm
          - generic [ref=e285]:
            - radio "2:00pm - 4:00pm" [ref=e286]
            - text: 2:00pm - 4:00pm
    - generic [ref=e287]:
      - button "Confirm" [ref=e288] [cursor=pointer]
...
> 204 |       expect(await boCalendarPage.isRescheduleDateSelectable(far)).toBe(true);
  ...
  277 |     });
  278 |     await boCalendarPage.closeRescheduleDialog(dialog);
  279 |   });
  280 | 
  281 |   test("BO reschedule beyond 6 days limit", async ({ boCalendarPage }) => {
  282 |     // "6 days" = the UCD-facing 6/day cap; CSE ignores it. Reuses whichever
  283 |     // full session (morning or afternoon) is found first as evidence the
  284 |     // day is at/near that cap.
  285 |     await boCalendarPage.navigate();
  286 |     const fullLabel = (await boCalendarPage.findDateWithSlotFull(MORNING)) ?? (await boCalendarPage.findDateWithSlotFull(AFTERNOON));
  287 |     if (!fullLabel) {
  288 |       test.skip(true, "No date at the daily booking limit available.");
  289 |       return;
  290 |     }
  291 |     const [d, m, y] = fullLabel.split("-");
  292 |     const iso = `${y}-${m}-${d}`;
  293 |     const dialog = await boCalendarPage.openRescheduleDialogOnly();
  294 |     if (!dialog) {
  295 |       test.skip(true, "No listed appointment to reschedule in this month");
  296 |       return;
  297 |     }
  298 |     await test.step(`Expected: CSE can still reschedule into ${fullLabel} despite it being at the UCD 6/day cap`, async () => {
  299 |       expect(await boCalendarPage.isRescheduleDateSelectable(iso)).toBe(true);
  300 |     });
  301 |     await boCalendarPage.closeRescheduleDialog(dialog);
  302 |   });
  303 | 
  304 |   // ── Add Appointment — existing-reference edge case ──
```