...
export const GEMINI_KEY_STORE = "gemini_api_key";
export function getGeminiKey(): string { try { return localStorage.getItem(GEMINI_KEY_STORE) ?? ""; } catch { return ""; } }
...
export default function SettingsModal({ onClose, onSaved, initial }: Props) {
  ...
  const [geminiKey, setGeminiKey] = useState(() => { try { return localStorage.getItem(GEMINI_KEY_STORE) ?? ""; } catch { return ""; } });
  ...
  function handleSave() {
    ...
    try { if (geminiKey.trim()) localStorage.setItem(GEMINI_KEY_STORE, geminiKey.trim()); else localStorage.removeItem(GEMINI_KEY_STORE); } catch {}
    onSaved(creds);
    onClose();
  }
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="pt-2 border-t border-slate-800">
            <p className="text-xs font-semibold text-slate-400 mb-3">AI Settings</p>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5">
                Gemini API Key
                <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="ml-2 text-blue-400 hover:underline">Get one free →</a>
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 pr-10 border border-slate-700 rounded-lg text-sm bg-slate-800 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-600"
                />
                <button type="button" onClick={() => setShowGeminiKey(v => !v)} className="absolute right-3 top-2.5 text-slate-500 hover:text-slate-300">
                  {showGeminiKey ? <EyeOff size={15} /> : <Eye size={15} />}
                </button>
              </div>
              <p className="text-xs text-slate-600 mt-1">Used for AI drafting on the Daily Update page. Free tier is more than enough.</p>
            </div>
          </div>
