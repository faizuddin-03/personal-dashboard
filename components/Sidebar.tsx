...
export default function Sidebar({ onSettingsClick, onLogout, isConnected }: Props) {
  ...
  return (
    <aside className="w-56 shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col h-screen sticky top-0">
      ...
      <div className="border-t border-slate-800 p-3 space-y-1">
        {importMsg && (
          <p className="text-xs text-center text-blue-400 pb-1">{importMsg}</p>
        )}
        ...
        <div className="flex gap-1">
          <button
            ...
            title="Export all dashboard data"
            className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs text-slate-400 hover:text-slate-100 hover:bg-slate-800 rounded-lg transition-colors"
          >
            <Download size={13} />
            Export
          </button>
        ...
        {isConnected && (
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-slate-500 hover:text-red-400 hover:bg-slate-800 rounded-lg transition-colors"
          >
            <LogOut size={15} />
            Disconnect Jira
          </button>
