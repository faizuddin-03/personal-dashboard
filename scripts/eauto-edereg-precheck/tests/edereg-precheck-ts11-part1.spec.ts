...
// ── CPC_E2E_TS11, PART 1 of 2 (EAINT-9306) ──
// "6-month SRD expiry" block, "pre-check done in step 2" entry point — the
// step2-first counterpart of CPC_E2E_TS5. Same shape as
...
// for the expiry patch. TS11's own payment-decline behaviour (RE
// reset-timer) is exercised entirely in Part 2, not here — Part 1 only
// needs to produce ONE real, persisted pre-check to later expire, and an
// inline-Failed outcome does not persist one (dead-end shape, same as
// CPC_E2E_TS2/TS8). Transaction ID looked up via the "eDereg Pre-Checking
// Transaction Listing" page (PrecheckEnquiryPage.findTransactionIdByVehicleNo),
// per Faizuddin 2026-08-24.
//
// NEVER RUN LIVE.
test('Deregistration (pre-check done in step 2) — CPC_E2E_TS11 Part 1 (Approved, then hand off for expiry patch)', async ({ loggedInPage: page, session, inputs }) => {
  ...
  console.log('RESULT:' + JSON.stringify({
    ...
    nextAction: 'Ask dev to patch this eDereg Pre-Checking transaction to expire (backdate JPJ approval past 6 months), then run CPC_E2E_TS11 Part 2 for the SAME vehicle no.',
    continuesAs: 'ts11-part2',
  }));
