...
export class EstmSession {
  // `fallback` is the fixture's original page. It is REQUIRED, not a nicety:
  // see active() below.
  ...
  active(): Page {
    ...
    // The `?? this.fallback` is load-bearing. eSERAHAN closes and reopens pages
    // on nearly every field, so if a window exists where EVERY page in the
    // context is closed, the filter yields [] and this returns undefined.
    // Callers do `this.active().waitForLoadState(...)`, which throws
    //   TypeError: Cannot read properties of undefined (reading 'waitForLoadState')
    // synchronously — the trailing .catch(() => {}) never sees it.
    // The reference spec this suite was refactored from always had the fallback
    // (`pages[pages.length - 1] ?? page`); it was dropped in the port and is
    // restored here. Do not remove it again.
  ...
  async closeBanners(maxRounds = 8): Promise<number> {
    // `[class*="dialog-campaign"]` is the important one: the Merdeka banner is
    // an <img class="dialog-campaign-1 dialog-campaign-frame">, so it has no
    // matching id and the numbered class means a second banner is
    // dialog-campaign-2. Matching on the class prefix catches every one.
    ...
    for (let round = 0; round < maxRounds; round++) {
      // Re-resolve every round — closing a banner can itself swap the page.
      const ap = this.active();
      if (!ap || ap.isClosed()) break;
      ...
      if (!clicked) {
        // Nothing matched a known close control. Log the banner's wrapper once
        // so the real X can be added to CLOSERS, then remove it outright — an
        // <img> banner sits inside an overlay that keeps eating clicks even if
        // only the image is hidden, so walk up to the modal ancestor first.
        if (round === 0) {
        ...
        await ap.evaluate(() => {
          ...
          document.querySelectorAll<HTMLElement>('[class*="dialog-campaign"], [id$="-campaign"], .modal-campaign, #eautoPopupOverlay, [class*="popup"]').forEach((el) => {
            // Climb to the outermost overlay ancestor so the backdrop goes too.
  ...
  async ensureChecked(selector: string): Promise<void> {
    ...
    // All three attempts failed. Fail HERE, naming the checkbox, rather than
    // returning quietly and letting the flow carry on with an unchecked box —
    // that surfaces several steps later as a Next button that does nothing,
    // which is a much harder failure to read. The reference spec asserts here
    // too; the assertion was dropped in the port.
   ...
   * wait for that dialog to close.
   *
   * Use this for EVERY eAuto popup instead of matching the button by text.
   * The portal appends dialogs to the end of <body>, and their button labels
   * collide with controls already on the page — the Payment step has a page
   * control reading "Next" (`#to-payment`) *and* a dialog confirm button
   * reading "Next". A `getByText('Next').first()` resolves to the page control,
   * in DOM order, so the dialog never gets clicked and the run hangs with the
   * popup open. See _reference/html/eauto/estm-step5-payment.html.
   *
   * The reliable hook is the class the portal puts on its own buttons:
   * `.confirm-dialog-btn` for the affirmative, `.cancel-dialog-btn` for the
   * negative — applied in each dialog's `open:` handler. One variant uses
   * `.confirm-wfw-dialog-btn`, so match both.
   *
   * ⚠️ There is a THIRD class, `.continue-dialog-btn` ("Save & Continue
   * Later"), on the payment dialog. It is NOT a cancel: it abandons the payment
   * and parks the transaction as a draft, which looks like progress and
   * silently produces no payment. Never click it. That is the reason this
   * matches on the confirm class rather than "the last button in the pane".
   *
   * Returns false when no dialog was showing.
   */
  async confirmDialog(timeout = 15_000): Promise<boolean> {
    const ap = await this.waitForActivePage();
    // Last in DOM order = topmost, which is the one stacked over the others.
    ...
    if (await confirm.count().catch(() => 0)) {
      ...
      // No class hook — fall back to label, but scoped INSIDE the dialog so it
      // can still never reach a page control of the same name. The label list
      // is deliberately exact-anchored and excludes anything containing
      // "Continue" / "Later" / "Back" / "Save".
  ...
  async clickYesAndWait(): Promise<void> {
    const ap = await this.waitForActivePage();
    // Bounded: this Yes is optional, and an unbounded click on a missing
    // element polls for the whole 30s actionTimeout before giving up.
